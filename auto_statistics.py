...
def process_excel_file(uploaded_file, stats_option):
    ...
    elif stats_option == "SBF NEW ENDO":
        summary_df["REMARKS"] = summary_df.apply(
...
def auto_statistics_section(stats_option="SBF NEGATIVE AUTOSTATS"):
    ...
    # Use stats_option passed from app.py
    ...
    if uploaded_file is not None:
        st.session_state[f"uploaded_file_{stats_option.lower().replace(' ', '_')}"] = uploaded_file
    ...
    if st.session_state.get(f"uploaded_file_{stats_option.lower().replace(' ', '_')}") is not None:
        try:
            # Process the file
            summary_df, output, removed_duplicates = process_excel_file(
                st.session_state[f"uploaded_file_{stats_option.lower().replace(' ', '_')}"], stats_option
            )
